...
void scale(float sx, float sy, float sz)
{
    for (int i = 0; i < 8; ++i)
    {
        scaled[i][0] = cube[i][0] * sx;
        scaled[i][1] = cube[i][1] * sy;
        scaled[i][2] = cube[i][2] * sz;
    }
}
...
void renderScene()
{
    ...
    // Draw scaled cube (green)
    drawCube(scaled, 0.0f, 1.0f, 0.0f);
...
int main()
{
    ...
    GLFWwindow *window = glfwCreateWindow(800, 600, "3D Cube Scaling", nullptr, nullptr);
    if (!window)
    {
    ...
    std::cout << "\nScaled Cube Coordinates:" << std::endl;
    for (int i = 0; i < 8; ++i)
    {
        std::cout << "Vertex " << i << ": (" << scaled[i][0] << ", "
                  << scaled[i][1] << ", " << scaled[i][2] << ")" << std::endl;
    }
